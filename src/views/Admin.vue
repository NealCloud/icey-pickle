          ...
          <ul class="space-y-2">
            ...
            <li>
                <a href="#" @click="dashboardSelect(dashboardKeys.mlpMembersList)" class="flex items-center p-2 text-base font-normal text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">
                  ...
                  <span class="flex-1 ml-3 whitespace-nowrap">MLP members</span>
...
export default {
  name: 'Home',
  components: { MLPmembers  },
  
  


  setup() {
    const dashboardBools = ref({mlpMembersList : true, todoList: false})
    const dashboardKeys = ref({mlpMembersList: 'mlp', todoList: 'todo'})
    const {user} = getUser()
